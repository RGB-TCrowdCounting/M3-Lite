...
def load_pretrain_S(net):
    pretrain_path = r'F:\CrowdCounting(rgbt)\RGBT Crowd Counting_lighting/mit_b0.pth'
...
class M3-Lite(nn.Module):
    def __init__(self, pretrained=True):
        super(M3-Lite, self).__init__()
        self.upsample_add = upsample_add
        self.upsample = cus_sample
        vgg_r = mit_b0()
        vgg_r = load_pretrain_S(vgg_r)
        vgg_d = mit_b0_d()##
        vgg_d = load_pretrain_S(vgg_d)

        self.layersR = vgg_r
        self.layersD = vgg_d


        self.trans8 = nn.Conv2d(256, 64, 1)
        self.trans4 = nn.Conv2d(160, 64, 1)
        self.trans2 = nn.Conv2d(64, 64, 1)
        self.trans1 = nn.Conv2d(32, 32, 1)

        self.t_trans2 = MIAFM(64,32)
        self.t_trans4 = SHAFM(160, 64)
        self.upconv16 = BasicDSConvConv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.upconv8 = BasicDSConvConv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.upconv4 = BasicDSConvConv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.upconv2 = BasicDSConvConv2d(64, 32, kernel_size=3, stride=1, padding=1)
        self.upconv1 = BasicDSConvConv2d(32, 32, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, RGBT):
        ...
        out_data_m = [out_data_1, out_data_2, out_data_4, out_data_8, dataD_2,dataR_2,dataD_4,dataR_4]
